/**
 * Admin.tsx — Code Arena Admin Dashboard
 * Developed by: Lucky
 * Project: Code Arena — Wildcard Draw System (Inferno'26)
 * Description: Secure admin panel for viewing all team draw results fetched
 *              from Google Apps Script backend. Protected by env-var admin key.
 */
...
export default function Admin() {
  ...
  // ✅ API URL — read from env var set in GitHub Secrets (VITE_SHEET_API_URL)
  // Falls back to the direct URL if the secret is not configured
  const API_URL =
    import.meta.env.VITE_SHEET_API_URL ||
    "https://script.google.com/macros/s/AKfycbyz94ngFAKYl8DLCRLU_-_Sq2u2bfNTqU_CGhxybtQfRuAYNa7pSImMABloxN7a1Rl_/exec";

  // ✅ Admin key — read from env var set in GitHub Secrets (VITE_ADMIN_KEY)
  // ⚠️  NEVER hardcode the real password here — use GitHub Actions secret
  const ADMIN_KEY = import.meta.env.VITE_ADMIN_KEY || "Lucky";
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Authenticate against the env-configured admin key
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col font-space-mono">
      ...
      {/* FOOTER */}
      <footer className="border-t border-neon-green/20 bg-black/40 py-4 sm:py-6 text-center space-y-1">
        <motion.p
        ...
        {/* Developer Credit */}
        <p className="text-[10px] text-neon-green/25 font-space-mono tracking-widest">
          DEVELOPED BY{" "}
          <span className="text-neon-green/40 font-bold">LUCKY</span>
