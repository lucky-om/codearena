/**
 * Wildcard.tsx — Code Arena Wildcard Draw Page
 * Developed by: Lucky
 * Project: Code Arena — Wildcard Draw System (Inferno'26)
 * Description: Main gameplay page — team verification, card shuffle animation,
 *              wildcard draw, and attack confirmation against opponent teams.
 *              Uses crypto.getRandomValues() for provably fair card selection.
 */
...
// Falls back to the hardcoded URL if the secret is not set
const API_URL =
...
export default function Wildcard() {
  ...
  return (
    <div className="min-h-screen grid-bg overflow-hidden text-white font-space-mono">
      ...
      {/* Footer — Developed by Lucky */}
      <footer className="border-t border-neon-cyan/30 backdrop-blur-sm text-center py-4 space-y-1">
        <motion.p
        ...
        <p className="text-[10px] text-neon-cyan/30 font-space-mono tracking-widest">
          DEVELOPED BY{" "}
          <span className="text-neon-cyan/50 font-bold">LUCKY</span>
        </p>
      </footer>
    </div>
  );
